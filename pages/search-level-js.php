...
if (isset($_POST['search_term_level'])) {
    $search_term_level =$_POST['search_term_level'];
	$search_term_level=$mysqli->real_escape_string($search_term_level);

    
    if (!empty($search_term_level)) {
        # code...
        $query = $mysqli->query("select * from members_tb where institutes_id='$institutes_id' AND admin_id='$admin_id' AND level like '%$search_term_level%'");
                ...
                <div class="box-body">
                <?php    echo '<p>Your search for  '. $search_term_level.'  returned '.  $result_count. '  result' .$suffix.'</p>' ; ?>
                </div><!-- /.box-body -->
              </div><!-- /.box -->
